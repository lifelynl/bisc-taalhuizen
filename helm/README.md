...
There are two ways to set values: to write them in the values file, which we recommend for values that do not have to be changed per cluster, and do not affect the security of the container. The other way is to set them using the ```--set``` option of ```helm instal``` or ```helm upgrade```. This way of setting values is not stored in the values.yaml file, and therefore ensures that secret values are not stored in the values.yaml file.
To use the ```--set``` option in your deployment, please note that the setting you want to override has to be defined (but not contain values) in the values.yaml file, and that you override them by writing down the yaml path of the setting, and divide the settings with a comma. For example, to set the api key and the pepper you write:
```CLI
