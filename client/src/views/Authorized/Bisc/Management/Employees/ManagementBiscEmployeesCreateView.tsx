...
export const ManagementBiscEmployeesCreateView: React.FunctionComponent<Props> = props => {
                ...
                RightComponent={
                    <Row>
                        <Button
                            ...
                            onClick={() => history.push(biscRoutes(organizationName).management.coworkers.index)}
                        >
                            {i18n._(t`Annuleren`)}
                        </Button>
    ...
    function handleCreate() {
        return async (e: React.FormEvent<HTMLFormElement>) => {
            ...
            try {
                if (input.person?.familyName && input.person.givenName && input.person.email) {
                    ...
                    history.push(
                        biscRoutes(organizationName).management.coworkers.detail(response.data?.createEmployee.id).data
                            .index
                    )
